...
function insights({ scope, count, expenseChange, top, daysLeft, remaining, hasBudget }: InsightInput): string {
  const money = (n: number) => formatMoney(n, scope.currency, scope.locale, 0);
  const lines: string[] = [];
...
export async function buildDashboard(scope: ReportScope, opts: { month?: string | null; from?: Date | null; to?: Date | null } = {}) {
  const tz = scope.timezone;
  const range = resolveRange(scope, { kind: 'CATEGORY', period: 'THIS_MONTH', ...opts });
  const now = Date.now();
  ...
  const [totals, prevTotals, slicesRaw, recent, trend] = await Promise.all([
    ...
    totalsByType(scope, previousRange(range, tz)),
    categoryRows(scope, range, 'EXPENSE'),
    Transaction.find({ trackerId: scope.trackerId }).sort({ occurredAt: -1 }).limit(6),
    buildReport(scope, { kind: 'DAILY', from: trendRange.from, to: trendRange.to }),
  ]);
  ...
  return {
    ...
    trend,
    recent,
    insight: insights({ scope, count: totals.count, expenseChange, top: slices[0], daysLeft, remaining, hasBudget: budget !== null }),
  };
}
