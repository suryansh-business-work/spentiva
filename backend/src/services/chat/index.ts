...
import { loadCtx, option, parseContext, sameName, withIds, type Ctx } from './context.js';
...
const say = (userId: UserDoc['_id'], text: string, kind: Kind = 'TEXT') => ChatMessage.create({ userId, role: 'ASSISTANT', kind, text });
...
async function runDraft(ctx: Ctx, draft: Draft): Promise<ChatMessageDoc> {
  ...
  try {
    ...
    if (step.kind === 'ASK') {
      return await ChatMessage.create({ userId, role: 'ASSISTANT', kind: 'OPTIONS', text: step.text, options: step.options, draft });
    }
    return await say(userId, step.text);
  } catch (err) {
    return say(userId, (err as Error).message, 'ERROR');
  }
}
...
async function runReport(ctx: Ctx, r: ParsedReport): Promise<ChatMessageDoc> {
  ...
  return ChatMessage.create({ userId: user._id, role: 'ASSISTANT', kind: 'REPORT', text, report, options: withIds(followUps) });
}
...
export async function sendMessage(user: UserDoc, text: string): Promise<ChatMessageOut[]> {
  ...
  const history = await ChatMessage.find({ userId: user._id }).sort({ createdAt: -1 }).limit(8);
  const userMsg = await ChatMessage.create({ userId: user._id, role: 'USER', kind: 'TEXT', text: trimmed });
  const ctx = await loadCtx(user);
  try {
    return await hydrate([userMsg, ...(await reply(ctx, trimmed, history.reverse()))]);
  } catch (err) {
    console.error('Chat reply failed:', err);
    return hydrate([userMsg, await say(user._id, (err as Error).message, 'ERROR')]);
  }
}

async function resolveChoice(user: UserDoc, msg: ChatMessageDoc, action: string, value: string | null): Promise<ChatMessageDoc> {
  if (action === 'UNDO') {
    const res = await Transaction.deleteOne({ _id: msg.transactionId, userId: user._id });
    return say(user._id, res.deletedCount ? 'Done — removed that entry.' : 'That entry was already removed.');
  }
  if (action === 'CANCEL') return say(user._id, 'Okay, cancelled. Nothing was logged.');
  ...
  try {
    ...
    return say(user._id, (err as Error).message, 'ERROR');
  }
}
...
export async function chooseOption(user: UserDoc, messageId: string, optionId: string): Promise<ChatMessageOut[]> {
  ...
  if (choice.action === 'PROMPT') return sendMessage(user, choice.value ?? choice.label);
  if (msg.resolved) throw badInput('This question was already answered');
  msg.resolved = true;
  msg.selectedOptionId = choice.id;
  await msg.save();
  return hydrate([msg, await resolveChoice(user, msg, choice.action, choice.value ?? null)]);
}

export async function clearChat(user: UserDoc) {
